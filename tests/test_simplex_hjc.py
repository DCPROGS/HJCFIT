...
The default local search is the *corrected* one, because code used to fit real
data should be correct. `local_search="fortran"` reproduces the original's bug
for anyone reproducing published results, and is pinned by a test of its own so
that correcting it cannot happen silently.
...
def test_finds_the_minimum_of_a_quadratic():
    r = simplex_hjc(quadratic([1.0, -2.0, 0.5]), [0.0, 0.0, 0.0], errfac=1e-6)
    assert r.success
...
def test_finds_the_rosenbrock_minimum():
    r = simplex_hjc(rosenbrock, [-1.2, 1.0], stpfac=np.e, errfac=1e-7,
                maxfev=100000)
    assert r.success
...
def test_linear_mode_scales_step_and_tolerance_with_the_guess():
    r = simplex_hjc(quadratic([10.0, 20.0]), [8.0, 25.0], logfit=False,
                errfac=1e-8)
    assert r.success
...
def test_log_mode_recovers_rate_constants():
    ...
    r = simplex_hjc(f, np.log([10.0, 1000.0, 5.0e7]), errfac=1e-7)
...
def test_args_are_passed_through():
    ...
    r = simplex_hjc(f, [0.0, 0.0], args=(np.array([3.0, -1.0]),), errfac=1e-6)
...
def test_starting_simplex_is_regular_and_fixed_in_log_space():
    ...
    seen = []
    f = quadratic([0.0, 0.0])
    simplex_hjc(lambda x: (seen.append(np.array(x, float)), f(x))[1],
...
def test_convergence_is_on_simplex_extent_alone():
    ...
    f = quadratic([0.0, 0.0])
    tight = simplex_hjc(f, [1.0, 1.0], errfac=1e-9)
    loose = simplex_hjc(f, [1.0, 1.0], errfac=1e-2)
...
def test_the_starting_simplex_costs_k_plus_one_evaluations():
    ...
    r = simplex_hjc(f, np.ones(4), maxfev=4)
    assert r.nfev >= 5
...
def test_restarts_are_counted_and_capped():
    r = simplex_hjc(rosenbrock, [-1.2, 1.0], stpfac=np.e, errfac=1e-9,
                nresmax=2, maxfev=100000)
    assert r.nrestarts <= 2


def test_no_restarts_when_nresmax_is_zero():
    r = simplex_hjc(rosenbrock, [-1.2, 1.0], stpfac=np.e, errfac=1e-6, nresmax=0)
...
def test_fortran_local_search_evaluates_the_unchanged_vertex():
    ...
    r = simplex_hjc(f, [1.0, 1.0], errfac=1e-3, local_search="fortran",
                nresmax=0)
...
def test_corrected_local_search_actually_perturbs():
    ...
    r = simplex_hjc(f, [1.0, 1.0], errfac=1e-3, local_search="corrected",
                nresmax=0)
    crt = 1e-3
...
def test_local_search_off_is_cheaper_and_lands_in_the_same_place():
    f = quadratic([0.4, -0.7])
    with_ls = simplex_hjc(f, [0.0, 0.0], errfac=1e-4, local_search="corrected",
                      nresmax=0)
    without = simplex_hjc(f, [0.0, 0.0], errfac=1e-4, local_search="off",
                      nresmax=0)
    assert without.nfev < with_ls.nfev
    npt.assert_allclose(without.fun, with_ls.fun, atol=1e-6)


def test_the_default_local_search_is_corrected():
    """Correctness is the default; fidelity to the original is opt-in.

    On the AChR fits of Colquhoun, Hatton & Hawkes (2003) the two are
    indistinguishable -- identical log-likelihoods and the same number of
    evaluations over 12 records -- so this costs nothing there. It is the
    default for problems where the local search does bite.
    """
    seen_default, seen_corrected = [], []

    def make(store):
        def f(x):
            store.append(np.array(x, float))
            return float(np.sum((np.asarray(x, float) - 0.3) ** 2))
        return f

    a = simplex_hjc(make(seen_default), [1.0, 1.0], errfac=1e-3, nresmax=0)
    b = simplex_hjc(make(seen_corrected), [1.0, 1.0], errfac=1e-3,
                    nresmax=0, local_search="corrected")
    assert a.nfev == b.nfev
    npt.assert_allclose(a.x, b.x, atol=0, rtol=0)
    assert len(seen_default) == len(seen_corrected)


def test_local_search_mode_is_validated():
    with pytest.raises(ValueError):
        simplex_hjc(quadratic([0.0]), [1.0], local_search="nonsense")


def test_empty_x0_is_rejected():
    with pytest.raises(ValueError):
        simplex_hjc(quadratic([]), [])
...
def test_result_uses_scipy_field_names():
    ...
    r = simplex_hjc(quadratic([1.0, 2.0]), [0.0, 0.0], errfac=1e-6)
    for name in ("x", "fun", "nfev", "nit", "success", "message"):
        assert hasattr(r, name), name
    assert isinstance(r, SimplexHJCResult)
...
def test_callback_sees_the_best_vertex():
    seen = []
    simplex_hjc(quadratic([2.0, 2.0]), [0.0, 0.0], callback=seen.append,
            errfac=1e-4)
    assert len(seen) > 1
...
def test_defaults_are_the_fortran_ones():
    assert SIMPLEX_HJC_DEFAULTS["stpfac_log"] == 5.0
    assert SIMPLEX_HJC_DEFAULTS["stpfac_lin"] == 0.2
    assert SIMPLEX_HJC_DEFAULTS["confac"] == 0.5
    assert SIMPLEX_HJC_DEFAULTS["resfac"] == 10.0
