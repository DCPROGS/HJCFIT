...
namespace HJCFIT {
  ...
  t_initvec MSWINDOBE vectors(IdealG const &, bool _initial = true);
  ...
  //!          problem is solved using Eigen's linear least-square utility, adding an extra row to
  //!          the matrix to impose the second condition.
  t_initvec MSWINDOBE vectors(MissedEventsG const &, bool _initial = true);
  ...
  //! vectors. However, to simplify the API, this function still deals with both cases.
  t_initvec MSWINDOBE CHS_vectors(MissedEventsG const &, t_real, bool _initial = true);
}
#endif 
