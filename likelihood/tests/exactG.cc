...
TEST_F(ExactGTest, out_of_range_Di) {

  StateMatrix transitions(Q, 2);
  ExactG exactG(transitions, 1e-4);
  EXPECT_THROW(exactG.D_af(-1), errors::Index);
  EXPECT_THROW(exactG.D_af(5), errors::Index);
  EXPECT_THROW(exactG.D_fa(-1), errors::Index);
  EXPECT_THROW(exactG.D_fa(5), errors::Index);
}

// Checks that Di matrices are what we expect them to be. 
TEST_F(ExactGTest, regression_Di_matrices) {
...
TEST_F(ExactGTest, regression_Ci11_matrices) {
  ...
  EXPECT_TRUE( ((exactG.recursion_af(0, 1, 1) - C011).array().abs() < 1e-8).all() );
  t_rmatrix C111(2, 2); C111 << -3.266094675374351e+01, -9.260126829437684e-01,
                                -1.234683577258275e-02, -3.500610868950084e-04;
  EXPECT_TRUE( ((exactG.recursion_af(1, 1, 1) - C111).array().abs() < 1e-8).all() );
  t_rmatrix C211(2, 2); C211 <<  1.292339253163696, -2.027634781642889,
                                -0.027035130421905,  0.04241716765587;
  EXPECT_TRUE( ((exactG.recursion_af(2, 1, 1) - C211).array().abs() < 1e-8).all() );
  t_rmatrix C311(2, 2); C311 <<  9.981021427566167e-04, 2.089946991412273e+00,
                                 2.786595988549777e-02, 5.834911732046284e+01;
  EXPECT_TRUE( ((exactG.recursion_af(3, 1, 1) - C311).array().abs() < 1e-8).all() );
  t_rmatrix C411(2, 2); C411 <<  9.430169806242189e-06, 7.072627354680796e-04,
                                 9.430169806241902e-06, 7.072627354680582e-04;
  EXPECT_TRUE( ((exactG.recursion_af(4, 1, 1) - C411).array().abs() < 1e-8).all() );
}

TEST_F(ExactGTest, out_of_range_Ciml_matrices) {
  StateMatrix transitions(Q, 2);
  ExactG exactG(transitions, 1e-4);
  
  EXPECT_THROW(exactG.recursion_af(-1, 0, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_af(5, 0, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(-1, 0, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(5, 0, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_af(0, 0, -1), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(0, 0, -1), errors::Index);
  EXPECT_THROW(exactG.recursion_af(0, -1, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(0, -1, 0), errors::Index);
  EXPECT_THROW(exactG.recursion_af(0, 0, 1), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(0, 0, 1), errors::Index);
  EXPECT_THROW(exactG.recursion_af(0, 1, 2), errors::Index);
  EXPECT_THROW(exactG.recursion_fa(0, 1, 2), errors::Index);
}
