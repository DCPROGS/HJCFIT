...
namespace DCProgs {
  ...
  t_rmatrix CHS_matrix_Hfa(MissedEventsG const &_g, t_real _tcrit) {
    return partial_CHS_matrix(*_g.asymptotes_fa_, _tcrit, _g.get_tau()) * _g.get_af_factor();
  }
  // CHS matrices \f$H_{FA}\f$
  t_rmatrix CHS_matrix_Haf(MissedEventsG const &_g, t_real _tcrit) {
    return partial_CHS_matrix(*_g.asymptotes_af_, _tcrit, _g.get_tau()) * _g.get_fa_factor();
  }
}
