...
def run_solver(maxiter=5000):
    ...
    options = {'xtol': 1e-4, 'ftol': 1e-4, 'maxiter': maxiter,
               'maxfev': 10000, 'disp': True}
    mysolver.run_optimizer(options)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Run GlyR4 simulation using MPI.')
    parser.add_argument("--maxiter", type=int,
                        help="Maximum number of iterations. "
                        "Defaults to 5000.",
                        default=5000)
    args = parser.parse_args()
    run_solver(args.maxiter)
